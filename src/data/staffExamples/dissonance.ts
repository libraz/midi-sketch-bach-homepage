...
export const dissonanceExamples: Record<string, StaffExampleDef> = {
  ...
  suspension23: {
    ruleIds: ['suspension_preparation', 'suspension_resolution_step_down'],
    verdict: 'caution',
    badge: { en: 'Teaching example', ja: '理論の譜例' },
    title: { en: 'Conventional 2-3 suspension', ja: '通常の2-3掛留' },
    diagnosis: {
      en: 'The bass is tied into the strong beat, then resolves down while the upper note holds.',
      ja: 'バスが強拍へタイで持ち越され、上声が保持されるあいだに下行して解決します。',
    },
    caption: {
      en: 'This illustrative two-bar example shows the conventional 2-3 figure below a cantus: a consonant preparation on the weak fourth beat, a bass note tied into the next bar’s strong beat, an upper step into the dissonant second, and a downward bass resolution on the following weak beat while the upper C holds. The engine’s named `Sus2_3` carrier currently uses the opposite bass direction (an ascending step), so this teaching example illustrates the interval relationship but is not an engine-accepted pattern.',
      ja: 'この2小節の説明用譜例は、定旋律の下に置く通常の2-3掛留を示します。弱い4拍目で協和する準備を置き、バスを次小節の強拍へタイで持ち越し、上声が動いて不協和な2度を作り、次の弱拍で上声の C を保持したままバスが下行して解決します。エンジンの `Sus2_3` という名前付きキャリアは現在、逆向き（下声の上行）を使うため、この理論の譜例は音程関係を示すもので、エンジンが受理するパターンではありません。',
    },
    time: '4/4',
    bars: 2,
    width: 620,
    ...
    upper: [
      ...
      { key: 'e/4' },
      { key: 'd/4' },
      { key: 'd/4', annotation: 'prep' },
      { key: 'c/4', annotation: '2', color: AMBER, issue: true, tie: true },
      { key: 'c/4', annotation: 'held', color: GREEN },
      { key: 'c/4' },
      { key: 'c/4' },
    ],
    lower: [
      ...
      { key: 'a/2', annotation: 'down', color: GREEN },
      { key: 'a/2' },
      { key: 'a/2' },
    ],
    issues: [
      { kind: 'bracket', label: 'prep - suspension - resolution', fromUpper: 3, toUpper: 5 },
      { kind: 'vertical', label: '2 -> 3', upperIndex: 4, lowerIndex: 4 },
      { kind: 'motion', label: 'bass resolves down', fromLower: 4, toLower: 5, color: GREEN },
    ],
  },
}
