...
function markPoints(issue: IssueMark, pos: Positions): Array<{ x: number; y: number }> {
  ...
  if (issue.upperIndex != null && pos.upper)
    points.push({ x: pos.upper.x(issue.upperIndex), y: pos.upper.y(issue.upperIndex) })
  if (issue.middleIndex != null && pos.middle)
    points.push({ x: pos.middle.x(issue.middleIndex), y: pos.middle.y(issue.middleIndex) })
  if (issue.lowerIndex != null && pos.lower)
    points.push({ x: pos.lower.x(issue.lowerIndex), y: pos.lower.y(issue.lowerIndex) })
  return points
}
    ...
    .map(issue => {
      ...
      if (issue.kind === 'vertical') {
        ...
        if (points.length >= 2) {
          ...
          const rings = points
            .map(
              p =>
                `<circle cx="${p.x}" cy="${p.y}" r="14" class="counterpoint-staff__svg-issue-ring" style="stroke: ${color}" />`,
            )
            .join('')
          return `
          ${label}
      ...
      if (issue.kind === 'note') {
        ...
        if (points.length > 0) {
          const rings = points
            .map(
              p =>
                `<circle cx="${p.x}" cy="${p.y}" r="18" class="counterpoint-staff__svg-issue-ring" style="stroke: ${color}" />`,
            )
            .join('')
          return `${label}${rings}`
        }
      ...
      if (issue.kind === 'bracket') {
        const part: VoicePart =
          issue.fromUpper != null ? 'upper' : issue.fromMiddle != null ? 'middle' : 'lower'
        const p = pos[part]
        if (!p) return label
