...
| 8 | シャコンヌ | `"chaconne"` | ソロ | ヴァイオリン | 2 | 3/4 | 16 | 16 |
| 9 | ゴルトベルク変奏曲 | `"goldberg_variations"` | 変奏 | チェンバロ | 3 | 4/4 | 20 | 20 |

::: info 小節数について
「スナップ前の基準小節数」は形式が持つ元の長さです。`scale` がこの値を倍率で伸ばし（約1倍/2倍/3倍/4倍）、形式ごとの小節単位へスナップします。そのため、フーガの基準値は42小節ですが、`scale: "short"` の出力は44小節です。`targetBars` は `scale` を上書きし、出力はすべての形式で128小節が上限です。
:::
...
::: info オルガン系（形式 0--6）
7つのオルガン形式はバッハのオルガン・レパートリーの主要ジャンルをカバーします。すべて既定でオルガンのレジストレーションが適用され、3声部を使用します。
:::
