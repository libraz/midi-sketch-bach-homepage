...
graph TD
    ...
    C --> D["3. 候補探索<br>（既定はキャリア再生）"]
    D --> E["4. 生成検証<br>（失敗をまとめて記録）"]
    E --> F["5. 初回レンダリング<br>（トラック）"]
    F --> G["6. 装飾処理"]
    G --> H["最終スコア検証"]
    H --> I["ベロシティ + 再レンダリング<br>CC + テンポ"]
    I --> J["7. MIDI + イベント出力<br>（出力調のピッチ）"]
```
...
graph TD
    ...
    B -->|"定旋律系"| E["固定されたコラール線<br>+ 装飾音型 + バス"]
    B -->|"線的/音型系"| F["連続的な音型"]
```
...
- **表現** — オルガンとチェンバロには CC 7 のレジストレーション段階、ピアノ、ヴァイオリン、チェロには CC 11 の連続表現を加えます。
