...
このページでは、MIDI Sketch Bachが正統的なバロックスタイルの対位法を生成するために使用する音楽理論のルールと、コンポーザーエンジンがそれをどう適用するかを説明します。

::: info エンジンによるルールの適用方法
ここでの対位法は2段階のプロセスです。**候補探索**が和声プランの和声音に固定しながら拍ごとにノートを選択し、各ノートが和声と他声部に対して協和するようにします。次に**ルールバリデータ**が組み上がった声部を以下の制約に対して検証し、違反があれば該当ルールを報告して即座に失敗させます。ルールは制約であり、探索はそれらを満たしつつ音楽的に魅力的な解を見つけます。
:::
...
パッサカリアとシャコンヌでは、グラウンドバスが楽曲全体を通じて（3/4拍子で）繰り返されます。追加の対位法制約が適用されます。

- グラウンドバスは不変であり、すべての変奏を通じて認識可能でなければならない
- 上声部は増大する独立性と複雑さを獲得する
- クライマックス（スパンの約80%）近くの変奏が最も密な書法を特徴とする
- 形式の固定された声部数はすべての変奏で保持される
